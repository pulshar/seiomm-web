...
// Hoisted navigation links array prevents recreating arrays and objects on every render
...
export function Navbar() {
  ...
  // Using pathname from hook ensures synchronized updates with client-side routing.
  const isLightPage = pathname !== '/' && pathname !== '/congreso';
  ...
  // Close mobile drawer when pathname changes to prevent stale open states
  useEffect(() => {
          ...
          >
            <div className="flex flex-col px-6 py-6 gap-4">
              ...
              <Link
                to="https://socios.seiomm.org/login"
                target="_blank"
                className={`flex items-center justify-center gap-2 px-5 py-3 rounded-full border-2 text-base font-medium ${effectiveIsScrolled ? 'border-seiomm-dark/50 bg-white text-seiomm-body' : 'border-white/50 text-white/80'
                  }`}
              >
                Área privada
              ...
              <Link
                to="https://socios.seiomm.org/register"
                target="_blank"
                className={`flex items-center justify-center gap-2 px-5 py-3 rounded-full border-2 border-transparent text-base font-medium  ${effectiveIsScrolled ? 'bg-seiomm-cyan text-white' : 'bg-white text-seiomm-dark'
                  }`}
              >
                Hazte socio
                <ArrowUpRight className="w-4 h-4" />
